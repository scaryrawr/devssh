...
# Find all browser sockets in /tmp (pattern: devssh-browser-*.sock)
# Sort by modification time (newest first) to prefer active sockets
BROWSER_SOCKETS=$(find /tmp -maxdepth 1 -name "devssh-browser-*.sock" -type s -print0 2>/dev/null | xargs -0 -r ls -t 2>/dev/null)
