...
if [ "${1:-}" = "send" ]; then
    ...
    __ns_socket=$(find /tmp -maxdepth 1 -name "devssh-notification-*.sock" -type s -exec ls -t {} + 2>/dev/null | head -1)
...
__find_notification_socket() {
    ...
    # Find all notification sockets in /tmp (pattern: devssh-notification-*.sock)
    ...
    local NOTIFICATION_SOCKETS=$(find /tmp -maxdepth 1 -name "devssh-notification-*.sock" -type s -exec ls -t {} + 2>/dev/null | head -1)
...
__send_notification() {
    ...
    local discovered_sockets=$(find /tmp -maxdepth 1 -name "devssh-notification-*.sock" -type s -exec ls -t {} + 2>/dev/null)
...
__notification_last_bash_command() {
    HISTTIMEFORMAT= history 1 | sed 's/^[ ]*[0-9]*[ ]*//'
}

__notification_last_zsh_command() {
    fc -ln -1 2>/dev/null || echo "unknown command"
}

__notification_handle_command_completion() {
    local exit_code="$1"
    local last_command_func="$2"
...
if [ -n "$BASH_VERSION" ]; then
    ...
    __notification_precmd() {
        ...
        __notification_in_command=0
        __notification_handle_command_completion "$exit_code" __notification_last_bash_command
    }
...
if [ -n "$ZSH_VERSION" ]; then
    ...
    __notification_precmd() {
        local exit_code=$?
        __notification_handle_command_completion "$exit_code" __notification_last_zsh_command
    }
